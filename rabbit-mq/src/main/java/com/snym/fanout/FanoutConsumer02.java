...
public class FanoutConsumer02 {
    ...
    public static void main(String[] args) {
        ...
        //声明交换机
        try {
            channel.exchangeDeclare(EXCHANGE_NAME, "com/snym/fanout");
            //生产临时队列
            String queue = channel.queueDeclare().getQueue();
            //绑定交换机和队列,扇形交换和队列的绑定路由键可以为""
            channel.queueBind(queue,EXCHANGE_NAME,"");
            System.out.println("FanoutConsumer02等待接收消息，把消息打印在控制台上......");
            DeliverCallback deliverCallback = (String consumerTag, Delivery message)->{
                String mesaage = new String(message.getBody());
                System.out.println("FanoutConsumer02控制台接收打印接收到的消息："+mesaage);
            };
